...
my_name = "D3V_SHAILU"
CHANNEL_ID = "-1002557558185"##change it with your channel 🆔 
...
async def start(client: Client, msg: Message):
    ...
    await start_message.edit_text(
        Data.START.format(msg.from_user.mention) +
        "Checking status Ok... Command Nhi Bataunga **Bot Made BY R𝐀𝐍𝐀 𝐉𝐈𝐈**🔍\n\n"
        "Progress:[🟩🟩🟩🟩🟩🟩🟩🟩🟩] 100%\n\n"
    )
...
@bot.on_message(filters.command(["RANAJII25", "RANA25", "D3V_SHAILU"]) )
async def txt_handler(bot: Client, m: Message):
    await m.delete()
    
    editable = await m.reply_text(f"**🔹Hi I am Poweful Rana TXT Downloader📥 Bot.**\n🔹**Send me the TXT file and wait.**")
    ...
    await m.reply_text("*✅ 𝐒𝐮𝐜𝐜𝐞𝐬𝐬𝐟𝐮𝐥𝐥𝐲 𝐃𝐨𝐧𝐞𓆪*")
...
@bot.on_message(filters.command(["Rana"]))
async def txt_handler(bot: Client, m: Message):
    editable = await m.reply_text(f"**🔹Send me the TXT file and wait.**")
    input: Message = await bot.listen(editable.chat.id)
    x = await input.download()
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            try:  
                ...
                cc = f'**🎞️ VID_ID: {str(count).zfill(3)}.\n\n Title: {name1} @ranajii25 {res}.mkv\n\n📚 Batch Name: {b_name}\n\n📥 Extracted By : {CR}\n\n**━━━━━✦𝐑𝐀𝐍𝐀_𝐉𝐈𝐈✦━━━━━**'
                cc1 = f'**📁 PDF_ID: {str(count).zfill(3)}.\n\n Title: {name1} @Ranajii25.pdf\n\n📚 Batch Name: {b_name}\n\n📥 Extracted By : {CR}\n\n**━━━━━✦𝐑𝐀𝐍𝐀_𝐉𝐈𝐈✦━━━━━**'
